...
import com.telenav.sdk.examples.util.VehicleInfoConfigHelper
...
open class MainActivity : AppCompatActivity() {
    ...
    private fun checkUserPermission() {
        ...
        permissionsRequired.add(storagePermission)
        ...
        permissionsRequired.forEach { permission ->
            if (ActivityCompat.checkSelfPermission(
                ...
                ActivityCompat.requestPermissions(this, permissionsRequired.toTypedArray(), permissionRequestCode)
            }
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && !Environment.isExternalStorageManager()) {
            startActivity(Intent(android.provider.Settings.ACTION_MANAGE_ALL_FILES_ACCESS_PERMISSION).apply {
...
//         TaLog.enableWriteLogsToFile(true)
    ...
    private suspend fun initSDK(options: SDKOptions, region: Region) : Boolean {
        ...
        withContext(Dispatchers.IO) {
            ...
            val adasLogDir = File(getExternalFilesDir(null), "adasLog")
            val vehicleInfoConfig = if (isTruck) VehicleInfoConfigHelper.createTruckInfoConfig() else VehicleInfoConfigHelper.createEVVehicleInfoConfig()
            val navSDKOptions = NavSDKOptions.builder(options, vehicleInfoConfig)
                ...
                .enableADASLog(true)
                .setADASLogStorePath(adasLogDir.absolutePath)
                .setTrafficFetchRange(3600)
                .setPositionEngineLogStorePath(peLogDir.absolutePath)
                .enableHDMap(true)
                .setMapStreamingSpaceLimit(1024 * 1024 * 1024L)
                .setBatteryConsumedCheckPercent(10)
                .setArrivalDistanceThreshold(20,30)
                .setChargingStationStatusCheckRemainingTime(300)
                .setIntersectionDisplayDistanceThreshold(400,180)
                .setDataViewName("tasdk-demo-lts4")
                .build()
